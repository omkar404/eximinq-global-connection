...
const processEvents = () => {
  ...
  return rawComplianceData.map((item, index) => {
    ...
    return {
      ...
      date: date,
      type:
        item.Event.includes("Payment") || item.Event.includes("Installment")
          ? "payment"
          : "filing",
...
export default function ComplianceCalendar() {
  ...
  // All events for current visible month
  ...
  // If a date is selected show that day's events, else show full month
  const agendaEvents = selectedDate
    ? events.filter((e) => isSameDay(e.date, selectedDate))
    : monthEvents;

  const sortedAgendaEvents = [...agendaEvents].sort((a, b) => a.date - b.date);

  // Risk counts from monthEvents
  const highRiskCount = monthEvents.filter((e) => e.risk === "high").length;
  const mediumRiskCount = monthEvents.filter((e) => e.risk === "medium").length;
  const lowRiskCount = monthEvents.filter((e) => e.risk === "low").length;
  ...
  const Badge = ({ children, className = "" }) => (
    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${className}`}>
      {children}
    </span>
  );
    ...
    <div className="min-h-screen bg-slate-50 p-0">
          ...
          <div className="md:flex md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-slate-500 max-w-2xl">
                Monitor critical filing deadlines for GST, DGFT, Customs, and FSSAI. Missing
                dates may result in penalties up to{" "}
                <span className="font-semibold text-red-600">500% of goods value</span>.
              </p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-7 gap-2">
                  {days.map((date, index) => {
                    ...
                    const hasHighRisk = dayEvents.some((e) => e.risk === "high");
          ...
          {/* Right Column: Agenda + Stats */}
          <div className="lg:w-1/3 space-y-6">

            {/* Daily / Monthly Agenda */}
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                  <LayoutDashboard className="h-4 w-4 text-blue-600" />
                  {selectedDate ? "Daily Agenda" : "Daily Agenda"}
                </h3>
                <div className="flex items-center gap-2">
                  <span className="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                    {sortedAgendaEvents.length} event{sortedAgendaEvents.length !== 1 ? "s" : ""}
                  </span>
                  {selectedDate ? (
                    <>
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-slate-600 text-xs font-bold bg-slate-100 hover:bg-slate-200 px-1.5 py-0.5 rounded"
                ...
                {sortedAgendaEvents.length === 0 ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-slate-500 text-sm">
                      No filings due {selectedDate ? "for this date" : "this month"}.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {sortedAgendaEvents.map((event, idx) => {
                      const showDateLabel =
                        !selectedDate &&
                        (idx === 0 ||
                          !isSameDay(sortedAgendaEvents[idx - 1].date, event.date));
                          ...
                          >
                            ...
                            {event.status === "completed" ? (
                              <div className="mt-2 flex items-center justify-center py-1.5 bg-emerald-100 text-emerald-700 rounded text-xs font-bold">
                                <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" /> Filed Successfully
                              </div>
                            ) : (
            ...
            <div className="bg-slate-900 rounded-xl shadow-lg p-5 text-white overflow-hidden relative">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700">
                  <div className="text-xs text-slate-400 mb-1">Pending Liability</div>
                ...
                <div className="col-span-2 bg-gradient-to-r from-red-900/50 to-slate-800 p-3 rounded-lg border border-red-900/30 flex items-center justify-between">
                  <div>
                    <div className="text-xs text-red-200 font-medium">Critical Alert</div>
                    <div className="text-sm font-bold text-white">IEC Renewal Pending</div>
                  </div>
            ...
            <div className="bg-blue-50 rounded-xl border border-blue-100 p-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-blue-800 uppercase">Need Regulatory Help?</p>
                <p className="text-sm font-medium text-slate-600">+91 74000 96950</p>
              </div>
