...
const complianceItems = [
  {
    title: "Solar PV Modules",
    href: "/services/scomet-licensing",  // ✅ Already correct
    target: "_blank",
    rel: "noopener noreferrer",
    subtitle: "ALMM & IEC 61215 / 61730",
    icon: <Sun className="w-6 h-6" />,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {complianceItems.map((item, idx) => (
          <a
            ...
            target={item.target}
            rel={item.rel}
            className="group p-6 rounded-xl border border-slate-200 hover:border-amber-500 hover:bg-amber-50 transition-all text-left relative overflow-hidden block"
          >
            {/* Decorative Corner */}
            <div className="absolute top-0 right-0 w-16 h-16 bg-amber-100 rounded-bl-full -mr-8 -mt-8 opacity-40 group-hover:opacity-100 transition-opacity" />
