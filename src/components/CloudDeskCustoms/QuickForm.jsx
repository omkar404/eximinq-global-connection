...
//     port.toLowerCase().includes(searchPort.toLowerCase())
//   );
...
//   const validate = () => {
//     const newErrors = {};
//     if (!form.licenseType) newErrors.licenseType = "Please select license type";
//     if (!form.port) newErrors.port = "Port of Import is required";
//     if (!form.dutyValue) newErrors.dutyValue = "License value is required";
//     if (!form.mobile || form.mobile.length !== 10)  // ✅ Added mobile validation
...
//         `${process.env.REACT_APP_API_URL}/api/customs-license-registration`,
//           // "http://localhost:5000/api/customs-license-registration",
//           {
//             method: "POST",
//             headers: { "Content-Type": "application/json" },
//             body: JSON.stringify({ ...form, type: "QUICK_FORM" }),
//           }
//         );
...
//       setForm({ licenseType: "", port: "", dutyValue: "", mobile: "" });
//       setSearchPort("");
//       setShowPorts(false);
//       setErrors({});
//     } catch (err) {
...
//         <h3 className="text-2xl font-bold text-brand-900">Registration Risk Check</h3>
//       </div>
//       <p className="text-slate-500 mb-6 text-sm">Facing issues submitting your Bond?</p>

//       <form onSubmit={handleSubmit}>
//         {/* License Type */}
//         <div className="mb-4">
//           <label className="block text-sm font-semibold mb-1">License Type</label>
//           <select
//             name="licenseType"
...
//           <label className="block text-sm font-semibold mb-1">Port of Import</label>
//           <input
//             type="text"
//             value={searchPort}
//             placeholder="Type port code or port name..."
//             onChange={(e) => { setSearchPort(e.target.value); setShowPorts(true); }}
//             onFocus={() => setShowPorts(true)}
//             onBlur={() => setTimeout(() => setShowPorts(false), 200)}
//             className={`w-full border rounded px-3 py-2 focus:outline-none focus:border-brand-500 ${
//               errors.port ? "border-red-500" : "border-slate-300"
//             }`}
//           />
//           {showPorts && (
//             <div className="absolute z-50 w-full bg-white border border-slate-300 rounded mt-1 max-h-60 overflow-y-auto shadow-lg">
...
//                 ))
//               ) : (
//                 <div className="px-3 py-2 text-slate-500">No ports found</div>
//               )}
//             </div>
//           )}
...
//         {/* License Value */}
//         <div className="mb-4">
//           <label className="block text-sm font-semibold mb-1">License Value (Duty Saved)</label>
//           <input
//             type="number"
//             name="dutyValue"
...
//             className={`w-full border rounded px-3 py-2 focus:outline-none focus:border-brand-500 ${
//               errors.dutyValue ? "border-red-500" : "border-slate-300"
//             }`}
//           />
//           {errors.dutyValue && <p className="text-red-500 text-xs mt-1">{errors.dutyValue}</p>}
//         </div>
...
//           }`}
//         >
//           {loading ? "Submitting..." : "Start Compliance"}
//         </button>
  ...
  return (
    ...
    <div className="bg-white text-slate-800 rounded-xl shadow-2xl p-4 md:p-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-xl font-bold text-brand-900">Registration Risk Check</h3>
      </div>
      <p className="text-slate-500 mb-4 text-xs">Facing issues submitting your Bond?</p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          {errors.licenseType && (
            <p className="text-red-500 text-xs mt-0.5">{errors.licenseType}</p>
          )}
        </div>
        ...
        <div className="mb-3 relative">
          ...
          />
          {showPorts && (
            <div className="absolute z-50 w-full bg-white border border-slate-300 rounded mt-1 max-h-40 overflow-y-auto shadow-lg text-sm">
              {filteredPorts.length > 0 ? (
                filteredPorts.map((port) => (
                  <div
                    key={port}
                    className="px-2 py-1.5 cursor-pointer hover:bg-slate-100"
                    onClick={() => {
                      setForm((prev) => ({ ...prev, port }));
                      setSearchPort(port);
                      setShowPorts(false);
                ...
                <div className="px-2 py-1.5 text-slate-500">No ports found</div>
              )}
            </div>
          )}
          {errors.port && (
        ...
        <div className="mb-3">
          <label className="block text-xs font-semibold mb-1">
            License Value (Duty Saved) <span className="text-red-500">*</span>
          </label>
          <input
            type="number"
            name="dutyValue"
          ...
          />
          {errors.dutyValue && (
            <p className="text-red-500 text-xs mt-0.5">{errors.dutyValue}</p>
          )}
        </div>
