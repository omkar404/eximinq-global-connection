...
const SERVICE_MAP = {
  Steel_Import_NOC_SIMS: { service: "Steel Import NOC SIMS" },
  Copper_NFMIMS: { service: "Copper NFMIMS" },
  Aluminium_NFMIMS: { service: "Aluminium NFMIMS" },
  Coal_CIMS: { service: "Coal CIMS" },
  Paper_PIMS: { service: "Paper_PIMS" },
  Chip_CHIMS: { service: "Chip_CHIMS" },
  IMS_Registration: { service: "IMS Registration" },
  NEED_HELP: {service: "NEED HELP"},
};
  ...
  const showimport = type === "IMS_Registration";
  ...
  const validate = () => {
    ...
    if (showimport && !category) newErrors.category = "Please select a Import service type.";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        ...
        // "http://localhost:5000/api/import-management-registration", // ✅ http:// is required   
        {
          method: "POST",
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {showimport && (
              <div>
                ...
                >
                  <option value="">Select import Service</option>
