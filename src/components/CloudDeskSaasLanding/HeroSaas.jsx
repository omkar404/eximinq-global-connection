...
const HeroSaas = ({ setShowModal }) => {
  return (
    ...
    //   </div>
    // </header>
    <header className="relative pt-32 pb-24 lg:pt-48 lg:pb-32 overflow-hidden bg-slate-900">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-slate-50"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-900/40 border border-blue-500/30 text-yellow-200 text-sm font-bold mb-8 backdrop-blur-sm animate-fade-in">
        ...
        <h1 className="text-3xl md:text-9xl font-extrabold text-white tracking-tight mb-8 leading-tight">
          EXIMINQ <br />
          <p className="text-3xl md:text-6xl text-slate-400 max-w-7xl mx-auto mb-12 font-light leading-relaxed">
            CLOUD DESK
          </p>
        </h1>

        <h1 className="text-5xl md:text-7xl font-extrabold text-white tracking-tight mb-8 leading-tight">
          Your Entire Trade Business.<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-sky-300">Audit-Ready in Seconds</span>
        </h1>

        <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-12 font-light leading-relaxed">
          Save 20+ hours per week on paperwork. Protect your business from retrospective audits. The first SaaS that replaces your messy email chains with a <strong>Digital Operating System</strong>
        </p>

        <div className="flex flex-col sm:flex-row justify-center gap-4 mb-16">
          <a href="#pricing" className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl shadow-lg shadow-blue-500/25 transition transform hover:-translate-y-1">
            See Pricing Plans
          </a>
          <button onclick="document.getElementById('highlights').scrollIntoView({behavior: 'smooth'})" className="px-8 py-4 bg-slate-800/50 hover:bg-slate-700/50 text-white font-bold rounded-xl border border-slate-700 backdrop-blur-sm transition">
            See How It Works
          </button>
        </div>

        {/* <!-- Stats/Trust --> */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto border-t border-slate-800/50 pt-8 text-slate-500 text-sm font-medium">
          <div><span className="text-white font-bold text-lg block">200+</span> Regulations Checks</div>
          <div><span className="text-white font-bold text-lg block">Zero</span> Email Chaos</div>
          <div><span className="text-white font-bold text-lg block">100%</span> OCR Searchable</div>
          <div><span className="text-white font-bold text-lg block">3-Year</span> Audit Shield</div>
        </div>
      </div>
    </header>
