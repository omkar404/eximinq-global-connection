...
export const ModalEnroll = ({ show, onClose, onSubmit, type }) => {
  ...
  const showPCB = type === "PCB_Consultancy";
  ...
  const PCB_OPTIONS = [
    "New - Consent to Establish (CTE)",
    "New - Consent to Operate (CTO)",
    "Renewal - Consent to Establish (CTE)",
    "Renewal - Consent to Operate (CTO)",
  ];
  ...
  const validate = () => {
    ...
    if (showPCB && !category) newErrors.category = "Please select a PCB service type.";
    return newErrors;
  };
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {showPCB && (
              <div>
                ...
                >
                  <option value="">Select PCB Service</option>
                  {PCB_OPTIONS.map((option) => (
