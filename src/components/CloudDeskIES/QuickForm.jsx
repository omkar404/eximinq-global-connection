...
const QuickForm = () => {
  ...
  const validate = () => {
    ...
    if (!form.companyStatus) {
      newErrors.companyStatus = "Please select a company status";
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-2.5">
          ...
          <select
            ...
            onChange={handleChange}
            className={`w-full border rounded px-2.5 py-1.5 text-sm focus:outline-none focus:border-brand-500 ${
              errors.companyStatus ? "border-red-500" : "border-slate-300"
            }`}
          >
          ...
          {errors.companyStatus && (
            <p className="text-red-500 text-xs mt-1">{errors.companyStatus}</p>
          )}
        </div>
