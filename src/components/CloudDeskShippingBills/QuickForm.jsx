...
const PORTS = [
  ...
  "INKAK1 - Custom House, Kakinada",
  "INVTZ1 - Custom House, Visakhapatnam",
...
// ─────────────────────────────────────────────────────
const PortSearchInput = ({ value, onChange, error }) => {
  ...
  const filtered = query.trim()
    ? PORTS.filter((p) => p.toLowerCase().includes(query.toLowerCase())).slice(0, 50)
    : PORTS.slice(0, 50);
...
const QuickForm = () => {
  ...
  const validate = () => {
    ...
    else if (!mobileRegex.test(form.mobile.trim())) newErrors.mobile = "Enter valid 10 digit Indian mobile number";
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        // "http://localhost:5000/api/shipping-bill-filing", // ✅ http:// is required        
        {
          method: "POST",
    ...
    <div className="bg-white text-slate-800 rounded-xl shadow-2xl p-4 md:p-5">
      ...
      <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="md:col-span-2">
          ...
          {errors.Incentive && (
            <p className="text-red-500 text-xs mt-1">{errors.Incentive}</p>
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="md:col-span-2 w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-2 text-sm rounded-lg transition flex items-center justify-center gap-2"
        >
