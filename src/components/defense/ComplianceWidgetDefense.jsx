...
const ITEMS = [
  {
    title: "Munitions List (Category 6)",
    href: "/services/scomet-licensing", // ✅ Fixed: = → :
    target: "_blank",
    rel: "noopener noreferrer",
    desc: "Arms, ammunition & controlled weapon systems",
  },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {ITEMS.map((item, idx) => (
            <Link
              ...
              target={item.target}
              rel={item.rel}
              className="group p-6 bg-slate-950 border border-slate-800 hover:border-blue-600 transition-all text-left relative overflow-hidden block"
            >
              <div className="absolute inset-0 bg-blue-900/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
