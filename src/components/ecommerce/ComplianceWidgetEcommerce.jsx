...
const ComplianceWidgetEcommerce = ({ setShowEnrollModal }) => {
  ...
  const complianceItems = [
    {
      ...
      tags: ["Value Limits", "KYC Norms", "Duty Exemption"],
    },
    {
      id: "commercial",
      title: "Commercial Export (CSB-V)",
    ...
    },
    {
      ...
      tags: ["Return to Origin", "Duty Free", "Notification 45/2017"],
    },
  ];
        ...
        {complianceItems.map((item) => (
          <Link
            ...
            className="group p-8 rounded-2xl bg-slate-50 
                       ...
                       hover:border-violet-500 
                       hover:bg-violet-50/60 
                       transition-all text-left relative overflow-hidden block cursor-pointer"
          >
            {/* Decorative Accent */}
            <div className="absolute top-0 right-0 w-24 h-24 
                            bg-violet-100 rounded-bl-full 
            ...
            <div className="flex justify-between items-center mb-4 relative z-10">
              <h3 className="font-bold text-xl text-slate-700 group-hover:text-violet-700 transition-colors">
                {item.title}
              </h3>
              <ChevronRight className="w-6 h-6 text-slate-300 group-hover:text-violet-500 transition-colors" />
            </div>
            ...
            <div className="flex flex-wrap gap-2 relative z-10">
              {item.tags.map((tag, idx) => (
                <span
                  key={idx}
                  className="px-2 py-1 bg-white/50 text-slate-400 text-xs rounded-full group-hover:bg-white/80 transition-colors"
                >
                  {tag}
                </span>
              ))}
