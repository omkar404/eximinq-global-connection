...
// export default function QuickFSSAI() {
...
export default function QuickFSSAI() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`✅ Thank you! Our compliance expert will contact you within 2 hours.`);
  ...
  return (
    <section id="contact" className="py-20 bg-white relative border-t border-slate-100">
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          <div className="w-full lg:w-1/2">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-6">File Before May 31st.</h2>
            <p className="text-slate-600 mb-8 text-lg">
              Stop worrying about FoSCoS portal formats. Hand over your data and let our team file your Form D-1 flawlessly before the deadline.
            </p>
            <div className="space-y-6">
              <div className="flex items-center p-4 bg-slate-50 rounded-lg border border-slate-100">
                <Mail className="w-8 h-8 text-sky-600 mr-4 shrink-0" />
          ...
          <div className="w-full lg:w-1/2">
            <div className="bg-slate-900 rounded-2xl p-8 shadow-2xl">
              <h3 className="text-2xl font-bold text-white mb-6">Request Filing Assistance</h3>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <input type="text" name="companyName" value={formData.companyName} onChange={handleChange}
                    placeholder="Company / FBO Name"
                    className={`w-full px-4 py-3 bg-slate-800 border rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:border-sky-500 ${errors.companyName ? 'border-red-500' : 'border-slate-700'}`} />
                ...
                <div>
                  <input type="text" name="licenseNumber" value={formData.licenseNumber} onChange={handleChange}
                    ...
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:border-sky-500" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <input type="email" name="email" value={formData.email} onChange={handleChange}
                      placeholder="Email Address"
                      className={`w-full px-4 py-3 bg-slate-800 border rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:border-sky-500 ${errors.email ? 'border-red-500' : 'border-slate-700'}`} />
                    {errors.email && <p className="text-red-400 text-xs mt-1">{errors.email}</p>}
                  </div>
                  <div>
                    <input type="tel" name="mobile" value={formData.mobile} onChange={handleChange}
                      placeholder="Mobile Number" maxLength={10}
                      className={`w-full px-4 py-3 bg-slate-800 border rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:border-sky-500 ${errors.mobile ? 'border-red-500' : 'border-slate-700'}`} />
                    {errors.mobile && <p className="text-red-400 text-xs mt-1">{errors.mobile}</p>}
                  </div>
                </div>
                <select name="requirement" value={formData.requirement} onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:border-sky-500">
                  <option value="">Select Requirement</option>
                  <option>FSSAI Annual Returns (Form D-1)</option>
                  <option>Nil Return Filing</option>
                  <option>License Renewal + Returns</option>
                </select>
                <button type="submit" disabled={loading}
                  className={`w-full py-4 text-white font-bold rounded-lg transition-colors text-lg mt-4 shadow-lg shadow-sky-900/50 ${loading ? 'bg-sky-800 cursor-not-allowed' : 'bg-sky-600 hover:bg-sky-500'}`}>
                  {loading ? "Submitting..." : "File Returns Now"}
                </button>
              </form>
            </div>
          </div>
