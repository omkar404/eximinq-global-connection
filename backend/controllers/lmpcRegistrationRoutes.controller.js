const lmpcRegistrationRoutes = require("../models/lmpcRegistrationRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "LMPC Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com,yadavsheshnath236@gmail.com",
    subject: `LMPC Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>LMPC Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>
          ${Category ? `<tr><td><b>Product Type</b></td><td>${Category}</td></tr>` : ""}
          ${company ? `<tr><td><b>Port of Import</b></td><td>${company}</td></tr>` : ""}
...
/* CREATE API */
exports.createlmpcRegistrationRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "LMPC Registration",
      mobile: mobile.trim(),
      Category: Category ? Category.trim() : null, // ✅ use the correct variable
      company: company ? company.trim() : null, // ✅ use correct variable
    ...
    const record = await lmpcRegistrationRoutes.create(recordData);
...
/* GET ALL */
exports.lmpcRegistrationRoutes = async (req, res) => {
  try {
    const data = await lmpcRegistrationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.lmpcRegistrationRoutesById = async (req, res) => {
  try {
    const data = await lmpcRegistrationRoutes.findById(req.params.id);
    if (!data) {
