...
const gemRegistrationRoutes = require("../models/gemRegistrationRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `Gem Registration — ${serviceDisplay}`,
    html: `
...
/* CREATE API */
exports.creategemRegistrationRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Gem Registration",
      mobile: mobile.trim(),
    ...
    const record = await gemRegistrationRoutes.create(recordData);
...
/* GET ALL */
exports.gemRegistrationRoutes = async (req, res) => {
  try {
    const data = await gemRegistrationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.gemRegistrationRoutesById = async (req, res) => {
  try {
    const data = await gemRegistrationRoutes.findById(req.params.id);
    if (!data) {
