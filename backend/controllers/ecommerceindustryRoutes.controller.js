const ecommerceindustryRoutes = require("../models/ecommerceindustryRoutes.model");
...
async function sendEmail(record) {
  try {
    ...
    const serviceDisplay = "ecommerce industry";

    await transporter.sendMail({
      from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
      to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
      subject: `New ecommerce industry  — ${serviceDisplay}`,
      html: `
        <div style="font-family:Arial;">
          <h2>New ecommerce industry</h2>
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createecommerceindustryRoutes = async (req, res) => {
  try {
    ...
    // ✅ Save to DB
    const record = await ecommerceindustryRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.getAllecommerceindustryRoutes = async (req, res) => {
  try {
    const data = await ecommerceindustryRoutes
      .find()
      .sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.ByIdecommerceindustryRoutes = async (req, res) => {
  try {
    const data = await ecommerceindustryRoutes.findById(req.params.id);
