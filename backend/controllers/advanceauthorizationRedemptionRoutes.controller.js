...
const advanceauthorizationRedemptionRoutes = require("../models/advanceauthorizationRedemptionRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "Advance Redemption Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `Advance Redemption Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Advance Redemption Registration</h2>
...
/* CREATE API */
exports.createadvanceauthorizationRedemptionRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Advance Redemption Registration",
      mobile: mobile.trim(),
      companyName: companyName ? companyName.trim() : null, // ✅ use the correct variable
      licenseNumber: licenseNumber ? licenseNumber.trim() : null, // ✅ use correct variable
    ...
    const record = await advanceauthorizationRedemptionRoutes.create(recordData);
...
/* GET ALL */
exports.advanceauthorizationRedemptionRoutes = async (req, res) => {
  try {
    const data = await advanceauthorizationRedemptionRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.advanceauthorizationRedemptionRoutesById = async (req, res) => {
  try {
    const data = await advanceauthorizationRedemptionRoutes.findById(req.params.id);
    if (!data) {
