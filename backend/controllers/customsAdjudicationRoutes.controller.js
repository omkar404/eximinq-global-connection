const customsAdjudicationRoutes = require("../models/customsAdjudicationRoutes.model");
    ...
    html: `
      <div style="font-family:Arial;">
        ...
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          ...
          ${issueCategory ? `<tr><td><b>Product Type</b></td><td>${issueCategory}</td></tr>` : ""}
          ${regionalAuthority ? `<tr><td><b>Port of Import</b></td><td>${regionalAuthority}</td></tr>` : ""}
...
/* CREATE API */
exports.createcustomsAdjudicationRoutes = async (req, res) => {
  try {
    ...
    const record = await customsAdjudicationRoutes.create(recordData);
...
/* GET ALL */
exports.customsAdjudicationRoutes = async (req, res) => {
  try {
    const data = await customsAdjudicationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.customsAdjudicationRoutesById = async (req, res) => {
  try {
    const data = await customsAdjudicationRoutes.findById(req.params.id);
    if (!data) {
