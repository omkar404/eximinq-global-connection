const engineeringindustryRoutes = require("../models/engineeringindustryRoutes.model");
...
async function sendEmail(record) {
  try {
    ...
    const serviceDisplay = "Engineering industry";

    await transporter.sendMail({
      from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
      to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
      subject: `New Engineering industry  — ${serviceDisplay}`,
      html: `
        <div style="font-family:Arial;">
          <h2>New Engineering industry</h2>
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createengineeringindustryRoutes = async (req, res) => {
  try {
    ...
    // ✅ Save to DB
    const record = await engineeringindustryRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.getAllengineeringindustryRoutes = async (req, res) => {
  try {
    const data = await engineeringindustryRoutes
      .find()
      .sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.getengineeringindustryRoutesById = async (req, res) => {
  try {
    const data = await engineeringindustryRoutes.findById(req.params.id);
