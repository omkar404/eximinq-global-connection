...
const warehouseLicenseRoutes = require("../models/warehouseLicenseRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "Warehouse License Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `Warehouse License Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Warehouse License Registration</h2>
...
/* CREATE API */
exports.createwarehouseLicenseRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Warehouse License Registration",
      mobile: mobile.trim(),
      Type: Type ? Type.trim() : null, // ✅ use the correct variable
      location: location ? location.trim() : null, // ✅ use correct variable
    ...
    const record = await warehouseLicenseRoutes.create(recordData);
...
/* GET ALL */
exports.warehouseLicenseRoutess = async (req, res) => {
  try {
    const data = await warehouseLicenseRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.warehouseLicenseRoutesById = async (req, res) => {
  try {
    const data = await warehouseLicenseRoutes.findById(req.params.id);
    if (!data) {
