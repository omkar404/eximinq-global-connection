...
const electronicsindustryRoutes = require("../models/electronicsindustryRoutes.model");
...
async function sendEmail(record) {
  try {
    ...
    const serviceDisplay = "Electronics industry";

    await transporter.sendMail({
      from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
      to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
      subject: `New Electronics industry  — ${serviceDisplay}`,
      html: `
        <div style="font-family:Arial;">
          <h2>New Electronics industry</h2>
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createelectronicsindustryRoutes = async (req, res) => {
  try {
    ...
    // ✅ Save to DB
    const record = await electronicsindustryRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.getAllelectronicsindustryRoutes = async (req, res) => {
  try {
    const data = await electronicsindustryRoutes
      .find()
      .sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.getelectronicsindustryRoutesById = async (req, res) => {
  try {
    const data = await electronicsindustryRoutes.findById(req.params.id);
