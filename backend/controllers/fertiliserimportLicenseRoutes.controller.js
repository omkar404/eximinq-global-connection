...
const fertiliserimportLicenseRoutes = require("../models/fertiliserimportLicenseRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "Fertilizer Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `Fertilizer Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Fertilizer Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          ...
          ${fertilizerType ? `<tr><td><b>Fertilizer Type</b></td><td>${fertilizerType}</td></tr>` : ""}
          ${state ? `<tr><td><b>State of Marketing</b></td><td>${state}</td></tr>` : ""}
...
/* CREATE API */
exports.createfertiliserimportLicenseRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Fertilizer Registration",
      mobile: mobile.trim(),
      fertilizerType: fertilizerType ? fertilizerType.trim() : null, // ✅ use the correct variable
    ...
    const record = await fertiliserimportLicenseRoutes.create(recordData);
...
/* GET ALL */
exports.fertiliserimportLicenseRoutes = async (req, res) => {
  try {
    const data = await fertiliserimportLicenseRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.fertiliserimportLicenseRoutesById = async (req, res) => {
  try {
    const data = await fertiliserimportLicenseRoutes.findById(req.params.id);
    if (!data) {
