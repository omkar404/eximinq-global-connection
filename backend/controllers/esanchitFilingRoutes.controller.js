const esanchitFilingRoutes = require("../models/esanchitFilingRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "E-Sanchit Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
    subject: `E-Sanchit Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>E-Sanchit Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>
          ${issueType ? `<tr><td><b>Product Type</b></td><td>${issueType}</td></tr>` : ""}
          ${companyName ? `<tr><td><b>Port of Import</b></td><td>${companyName}</td></tr>` : ""}
...
/* CREATE API */
exports.createesanchitFilingRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "E-Sanchit Registration",
      mobile: mobile.trim(),
    ...
    const record = await esanchitFilingRoutes.create(recordData);
...
/* GET ALL */
exports.esanchitFilingRoutes = async (req, res) => {
  try {
    const data = await esanchitFilingRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.esanchitFilingRoutesById = async (req, res) => {
  try {
    const data = await esanchitFilingRoutes.findById(req.params.id);
    if (!data) {
