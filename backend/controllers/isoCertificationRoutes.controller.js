...
const isoCertificationRoutes  = require("../models/isoCertificationRoutes.model");
    ...
    html: `
      <div style="font-family:Arial;">
        ...
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          ...
          ${standard ? `<tr><td><b>ISO Standard</b></td><td>${standard}</td></tr>` : ""}
          ${accreditation ? `<tr><td><b>Accreditation Type</b></td><td>${accreditation}</td></tr>` : ""}
...
/* CREATE API */
exports.createisoCertificationRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "ISO Registration",
      mobile: mobile.trim(),
      standard: standard ? standard.trim() : null, // ✅ use the correct variable
      accreditation: accreditation ? accreditation.trim() : null, // ✅ use correct variable
    ...
    const record = await isoCertificationRoutes.create(recordData);
...
/* GET ALL */
exports.isoCertificationRoutes  = async (req, res) => {
  try {
    const data = await isoCertificationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.isoCertificationRoutesById = async (req, res) => {
  try {
    const data = await isoCertificationRoutes.findById(req.params.id);
    if (!data) {
