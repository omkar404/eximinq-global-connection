const eprannualReturnsRoutes = require("../models/eprannualReturnsRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "EPR Annual Returns Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
    subject: `EPR Annual Returns Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>EPR Annual Returns Registration</h2>
...
/* CREATE API */
exports.createeprannualReturnsRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "EPR Annual Returns Registration",
      mobile: mobile.trim(),
    ...
    const record = await eprannualReturnsRoutes.create(recordData);
...
/* GET ALL */
exports.eprannualReturnsRoutes = async (req, res) => {
  try {
    const data = await eprannualReturnsRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.eprannualReturnsRoutesById = async (req, res) => {
  try {
    const data = await eprannualReturnsRoutes.findById(req.params.id);
    if (!data) {
