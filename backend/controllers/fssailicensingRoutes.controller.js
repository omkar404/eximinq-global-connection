...
const fssailicensingRoutes = require("../models/fssailicensingRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "fssai licensing Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `fssai licensing Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>fssai licensing Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          ...
          ${productType ? `<tr><td><b>Product Type</b></td><td>${productType}</td></tr>` : ""}
          ${portOfImport ? `<tr><td><b>Port of Import</b></td><td>${portOfImport}</td></tr>` : ""}
...
/* CREATE API */
exports.createfssailicensingRoutes = async (req, res) => {
  try {
    ...
    const {
      ...
      category,
      issue,
      productType,     // ✅ camelCase
      portOfImport,    // ✅ camelCase
    } = req.body;
    ...
    const recordData = {
      service: service || "fssai licensing Registration",
      mobile: mobile.trim(),
      productType: productType ? productType.trim() : null,   // ✅ use the correct variable
      portOfImport: portOfImport ? portOfImport.trim() : null, // ✅ use correct variable
    ...
    const record = await fssailicensingRoutes.create(recordData);
...
/* GET ALL */
exports.fssailicensingRoutes = async (req, res) => {
  try {
    const data = await fssailicensingRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.fssailicensingRoutesById = async (req, res) => {
  try {
    const data = await fssailicensingRoutes.findById(req.params.id);
    if (!data) {
