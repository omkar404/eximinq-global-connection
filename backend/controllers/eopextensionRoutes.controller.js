...
const eopextensionRoutes = require("../models/eopextensionRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `New EOP Extension Registration — ${type}`, // ✅ Fix: SCOMET → EOP Extension
    html: `
  ...
  console.log("Email sent for EOP Extension:", _id); // ✅ Fix: SCOMET → EOP Extension
}

exports.createeopextensionRoutes = async (req, res) => {
  try {
    console.log("EOP Extension Incoming:", req.body); // ✅ Fix: SCOMET → EOP Extension
    ...
    const record = await eopextensionRoutes.create({
      name:     name.trim(),
      mobile:   mobile.trim(),
    ...
    console.log("Saved EOP Extension record:", record._id); // ✅ Fix: SCOMET → EOP Extension
    ...
    console.error("EOP Extension Error:", error.message); // ✅ Fix: SCOMET → EOP Extension
