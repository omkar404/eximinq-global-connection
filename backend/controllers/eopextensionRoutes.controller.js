...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `EOP Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>EOP Registration</h2>
...
exports.createeopextensionRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "EOP Registration",
      mobile: mobile.trim(),
    ...
    const record = await eopextensionRoutes.create(recordData);
...
/* GET ALL */
exports.eopextensionRoutes = async (req, res) => {
  try {
    const data = await eopextensionRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.eopextensionRoutesById = async (req, res) => {
  try {
    const data = await eopextensionRoutes.findById(req.params.id);
    if (!data) {
