const freesaleCertificateRoutes = require("../models/freesaleCertificateRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `free-sale-certificate Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>free-sale-certificate Registration</h2>
...
/* CREATE API */
exports.createfreesaleCertificateRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "free-sale-certificate Registration",
      mobile: mobile.trim(),
    ...
    const record = await freesaleCertificateRoutes.create(recordData);
...
/* GET ALL */
exports.freesaleCertificateRoutes = async (req, res) => {
  try {
    const data = await freesaleCertificateRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.freesaleCertificateRoutesById = async (req, res) => {
  try {
    const data = await freesaleCertificateRoutes.findById(req.params.id);
    if (!data) {
