const wpcLicenseRoutes  = require("../models/wpcLicenseRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "WPC Registration";
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:Arial;">
        <h2>WPC Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>
          ${productName ? `<tr><td><b>Product Name</b></td><td>${productName}</td></tr>` : ""}
          ${frequency ? `<tr><td><b>Frequency Band</b></td><td>${frequency}</td></tr>` : ""}
...
/* CREATE API */
exports.createwpcLicenseRoutes  = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "WPC Registration",
      mobile: mobile.trim(),
      productName: productName ? productName.trim() : null, // ✅ use the correct variable
      frequency: frequency ? frequency.trim() : null, // ✅ use correct variable
    ...
    const record = await wpcLicenseRoutes.create(recordData);
...
/* GET ALL */
exports.wpcLicenseRoutes  = async (req, res) => {
  try {
    const data = await wpcLicenseRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.wpcLicenseRoutesById = async (req, res) => {
  try {
    const data = await wpcLicenseRoutes.findById(req.params.id);
    if (!data) {
