...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "EPCG Redemption Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `EPCG Redemption Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>EPCG Redemption Registration</h2>
...
/* CREATE API */
exports.createepcgRedemptionRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "EPCG Redemption Registration",
      mobile: mobile.trim(),
    ...
    const record = await epcgRedemptionRoutes.create(recordData);
...
/* GET ALL */
exports.epcgRedemptionRoutes = async (req, res) => {
  try {
    const data = await epcgRedemptionRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.epcgRedemptionRoutesById = async (req, res) => {
  try {
    const data = await epcgRedemptionRoutes.findById(req.params.id);
    if (!data) {
