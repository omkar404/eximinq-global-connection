const textilesindustryRoutes = require("../models/textilesindustryRoutes.model");
...
async function sendEmail(record) {
  try {
    ...
    const serviceDisplay = "Textiles industry";

    await transporter.sendMail({
      from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
      to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
      subject: `New Textiles industry  — ${serviceDisplay}`,
      html: `
        <div style="font-family:Arial;">
          <h2>New Textiles industry</h2>
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createtextilesindustryRoutes = async (req, res) => {
  try {
    ...
    // ✅ Save to DB
    const record = await textilesindustryRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.getAlltextilesindustryRoutes = async (req, res) => {
  try {
    const data = await textilesindustryRoutes
      .find()
      .sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.ByIdtextilesindustryRoutes = async (req, res) => {
  try {
    const data = await textilesindustryRoutes.findById(req.params.id);
