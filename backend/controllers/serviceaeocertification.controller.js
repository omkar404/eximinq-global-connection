...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "AEO Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, yadavsheshnath236@gmail.com",
    subject: `AEO Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>AEO Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          ...
          ${yearsInBusiness ? `<tr><td><b>Years in Business</b></td><td>${yearsInBusiness}</td></tr>` : ""}
...
/* CREATE API */
exports.createserviceaeocertification = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "AEO Registration",
      mobile: mobile.trim(),
      businessType: businessType ? businessType.trim() : null, // ✅ use the correct variable
      yearsInBusiness: yearsInBusiness ? yearsInBusiness.trim() : null, // ✅ use correct variable
    ...
    const record = await serviceaeocertification.create(recordData);
...
exports.serviceaeocertification = async (req, res) => {
  try {
    const data = await serviceaeocertification.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.serviceaeocertificationById = async (req, res) => {
  try {
    const data = await serviceaeocertification.findById(req.params.id);
    if (!data) {
