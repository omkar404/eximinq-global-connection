const billOfEntryFilingRoutes = require("../models/billOfEntryFilingRoutes.model");
...
/* CREATE API */
exports.createbillOfEntryFilingRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Bill of Entry Registration",
      mobile: mobile.trim(),
      port: port ? port.trim() : null, // ✅ use the correct variable
      cargo: cargo ? cargo.trim() : null, // ✅ use correct variable
    ...
    const record = await billOfEntryFilingRoutes.create(recordData);
...
/* GET ALL */
exports.billOfEntryFilingRoutes = async (req, res) => {
  try {
    const data = await billOfEntryFilingRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.billOfEntryFilingRoutesById = async (req, res) => {
  try {
    const data = await billOfEntryFilingRoutes.findById(req.params.id);
    if (!data) {
