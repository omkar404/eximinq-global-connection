...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "Duty Drawback Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `Duty Drawback Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Duty Drawback Registration</h2>
...
exports.createdutydrawbackRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Duty Drawback Registration",
      mobile: mobile.trim(),
      exportProduct: exportProduct ? exportProduct.trim() : null, // ✅ use the correct variable
      claimType: claimType ? claimType.trim() : null, // ✅ use correct variable
    ...
    const record = await dutydrawbackRoutes.create(recordData);
...
/* GET ALL */
exports.dutydrawbackRoutes = async (req, res) => {
  try {
    const data = await dutydrawbackRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.dutydrawbackRoutesById = async (req, res) => {
  try {
    const data = await dutydrawbackRoutes.findById(req.params.id);
    if (!data) {
