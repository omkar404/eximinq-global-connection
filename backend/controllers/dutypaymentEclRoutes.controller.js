...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "Duty Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `Duty Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Duty Registration</h2>
...
/* CREATE API */
exports.createdutypaymentEclRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Duty Registration",
      mobile: mobile.trim(),
      hsn: hsn ? hsn.trim() : null, // ✅ use the correct variable
      cifValue: cifValue ? cifValue.trim() : null, // ✅ use correct variable
    ...
    const record = await dutypaymentEclRoutes.create(recordData);
...
/* GET ALL */
exports.dutypaymentEclRoutes = async (req, res) => {
  try {
    const data = await dutypaymentEclRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.dutypaymentEclRoutesById = async (req, res) => {
  try {
    const data = await dutypaymentEclRoutes.findById(req.params.id);
    if (!data) {
