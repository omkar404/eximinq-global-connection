...
const defenseindustryRoutes = require("../models/defenseindustryRoutes.model");
...
async function sendEmail(record) {
  try {
    ...
    const serviceDisplay = "Defense industry";

    await transporter.sendMail({
      from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
      to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
      subject: `New Defense industry  — ${serviceDisplay}`,
      html: `
        <div style="font-family:Arial;">
          <h2>New Defense industry</h2>
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createdefenseindustryRoutes = async (req, res) => {
  try {
    ...
    // ✅ Save to DB
    const record = await defenseindustryRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.getAlldefenseindustryRoutes = async (req, res) => {
  try {
    const data = await defenseindustryRoutes
      .find()
      .sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.ByIddefenseindustryRoutes = async (req, res) => {
  try {
    const data = await defenseindustryRoutes.findById(req.params.id);
