const fssaiannualReturnsRoutes = require("../models/fssaiannualReturnsRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "FSSAI Annual Returns Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `FSSAI Annual Returns Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>FSSAI Annual Returns Registration</h2>
...
/* CREATE API */
exports.createfssaiannualReturnsRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "FSSAI Annual Returns Registration",
      mobile: mobile.trim(),
    ...
    const record = await fssaiannualReturnsRoutes.create(recordData);
...
/* GET ALL */
exports.fssaiannualReturnsRoutes = async (req, res) => {
  try {
    const data = await fssaiannualReturnsRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.fssaiannualReturnsRoutesById = async (req, res) => {
  try {
    const data = await fssaiannualReturnsRoutes.findById(req.params.id);
    if (!data) {
