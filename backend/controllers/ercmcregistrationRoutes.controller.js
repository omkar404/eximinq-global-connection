...
const ercmcregistrationRoutes = require("../models/ercmcregistrationRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `E-RCMC Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>E-RCMC Registration</h2>

        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>

           ${product ? `<tr><td><b>Product</b></td><td>${product}</td></tr>` : ""}
           ${hsnCode ? `<tr><td><b>HSN Code</b></td><td>${hsnCode}</td></tr>` : ""}
...
/* ─────────────────────────────────────────────
   CREATE API
───────────────────────────────────────────── */
exports.createercmcregistrationRoutes = async (req, res) => {
  try {
    ...
    // ✅ Prepare Data
    const recordData = {
      service: service || "E-RCMC Registration",
      mobile: mobile.trim(),
    ...
    // ✅ Save to DB
    const record = await ercmcregistrationRoutes.create(recordData);
...
/* ─────────────────────────────────────────────
   GET ALL
───────────────────────────────────────────── */
exports.ercmcregistrationRoutes = async (req, res) => {
  try {
    const data = await ercmcregistrationRoutes.find().sort({ createdAt: -1 });
...
/* ─────────────────────────────────────────────
   GET BY ID
───────────────────────────────────────────── */
exports.ercmcregistrationRoutesById = async (req, res) => {
  try {
    const data = await ercmcregistrationRoutes.findById(req.params.id);
