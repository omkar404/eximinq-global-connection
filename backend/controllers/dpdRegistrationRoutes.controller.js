...
const dpdRegistrationRoutes = require("../models/dpdRegistrationRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `DPD Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>DPD Registration</h2>
...
/* CREATE API */
exports.createdpdRegistrationRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "DPD Registration",
      mobile: mobile.trim(),
    ...
    const record = await dpdRegistrationRoutes.create(recordData);
...
/* GET ALL */
exports.dpdRegistrationRoutes = async (req, res) => {
  try {
    const data = await dpdRegistrationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.dpdRegistrationRoutesById = async (req, res) => {
  try {
    const data = await dpdRegistrationRoutes.findById(req.params.id);
    if (!data) {
