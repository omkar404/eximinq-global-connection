const rodtepschemeRoutes = require("../models/rodtepschemeRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `Rodtep Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>Rodtep Registration</h2>
...
/* CREATE API */
exports.createrodtepschemeRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "Rodtep Registration",
      mobile: mobile.trim(),
    ...
    const record = await rodtepschemeRoutes.create(recordData);
...
/* GET ALL */
exports.rodtepschemeRoutes = async (req, res) => {
  try {
    const data = await rodtepschemeRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.rodtepschemeRoutesById = async (req, res) => {
  try {
    const data = await rodtepschemeRoutes.findById(req.params.id);
    if (!data) {
