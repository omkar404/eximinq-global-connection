const dscServiceRoutes = require("../models/dscServicesRoutes.model");
...
async function sendEmail(record) {
  ...
  const serviceDisplay = service || "DSC Registration";

  await transporter.sendMail({
    from: `"EXIMINQ CloudDesk" <${process.env.SMTP_USER}>`,
    to: "crm@eximinq.com, omkarmhetar100@gmail.com, sheshnathyadav1827499@gmail.com",
    subject: `DSC Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>DSC Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>
          ${dscType ? `<tr><td><b>DSC Type</b></td><td>${dscType}</td></tr>` : ""}
          ${validity ? `<tr><td><b>Validity</b></td><td>${validity}</td></tr>` : ""}
...
exports.createdscServiceRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "DSC Registration",
      mobile: mobile.trim(),
      dscType: dscType ? dscType.trim() : null, // ✅ use the correct variable
      validity: validity ? validity.trim() : null, // ✅ use correct variable
    ...
    const record = await dscServiceRoutes.create(recordData);
...
/* GET ALL */
exports.dscServiceRoutes = async (req, res) => {
  try {
    const data = await dscServiceRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.dscServiceRoutesById = async (req, res) => {
  try {
    const data = await dscServiceRoutes.findById(req.params.id);
    if (!data) {
