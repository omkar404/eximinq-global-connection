const svbRegistrationRoutes = require("../models/svbRegistrationRoutes.model");
...
async function sendEmail(record) {
  ...
  await transporter.sendMail({
    ...
    subject: `SVB Registration — ${serviceDisplay}`,
    html: `
      <div style="font-family:Arial;">
        <h2>SVB Registration</h2>
        <table border="1" cellpadding="6" style="border-collapse:collapse;">
          <tr><td><b>Submission Type</b></td><td>${type}</td></tr>
          <tr><td><b>Service</b></td><td>${serviceDisplay}</td></tr>
          ${relationshipType ? `<tr><td><b>Product Type</b></td><td>${relationshipType}</td></tr>` : ""}
          ${importValue ? `<tr><td><b>Port of Import</b></td><td>${importValue}</td></tr>` : ""}
...
/* CREATE API */
exports.createsvbRegistrationRoutes = async (req, res) => {
  try {
    ...
    const recordData = {
      service: service || "SVB Registration",
      mobile: mobile.trim(),
    ...
    const record = await svbRegistrationRoutes.create(recordData);
...
/* GET ALL */
exports.svbRegistrationRoutes = async (req, res) => {
  try {
    const data = await svbRegistrationRoutes.find().sort({ createdAt: -1 });
...
/* GET BY ID */
exports.svbRegistrationRoutesById = async (req, res) => {
  try {
    const data = await svbRegistrationRoutes.findById(req.params.id);
    if (!data) {
